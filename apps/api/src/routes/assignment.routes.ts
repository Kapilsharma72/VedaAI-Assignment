...
router.get('/:id', authenticate, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const assignment = await assignmentService.getById(req.userId!, req.params.id);
...
router.post('/:id/regenerate', authenticate, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const assignment = await assignmentService.regenerate(req.userId!, req.params.id);
        res.status(200).json({ success: true, data: assignment });
    }
    catch (err) {
        next(err);
