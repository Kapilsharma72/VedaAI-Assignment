...
async function processGenerationJob(job: Job<GenerationJobData>): Promise<void> {
    ...
    if (!parsed) {
        ...
        if (!parsed) {
            ...
            assignment.status = 'failed';
            await assignment.save();
            emitFailed(assignmentId, 'Failed to generate a valid question paper after multiple attempts. Please try regenerating.');
    ...
    const paperDoc = await GeneratedPaper.findOneAndUpdate({ assignmentId }, {
        assignmentId,
