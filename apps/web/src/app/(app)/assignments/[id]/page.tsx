...
function LoadingSkeleton() {
  return (
    <div className="animate-pulse space-y-4" aria-label="Loading assignment" aria-busy="true">
        ...
        {connectionError && (
          <div className="w-full rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            ...
            <button
              ...
              className="mt-2 rounded-md bg-red-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2"
            >
              Retry Connection
            </button>
          </div>
...
function ErrorCard({ message, onRetry, isRetrying }: ErrorCardProps) {
  return (
    <div
      className="rounded-xl border border-red-200 bg-red-50 p-8 shadow-sm"
      role="alert"
      aria-live="assertive"
    >
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <button
          ...
          onClick={onRetry}
          disabled={isRetrying}
          className="flex items-center gap-2 rounded-lg bg-red-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 disabled:opacity-50"
        >
          {isRetrying ? (
            <>
              <svg
...
function LiveGenerationView({
  assignmentId,
  subject,
  socketKey: _socketKey,
  onRegenerateStart,
}: LiveGenerationViewProps) {
  const { progress, message, paper, error, isConnected, retryConnection } =
...
export default function AssignmentPage({ params }: AssignmentPageProps) {
  ...
  useEffect(() => {
    ...
    async function fetchAssignment() {
      ...
      try {
        ...
        if (!cancelled) {
          const msg =
            err instanceof Error ? err.message : 'Failed to load assignment. Please try again.';
          setFetchError(msg);
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {assignment.status === 'completed' && !paper && fetchError && (
        <div
          className="rounded-xl border border-red-200 bg-red-50 p-8 text-center shadow-sm"
          role="alert"
        >
      ...
      {assignment.status === 'failed' && (
        <ErrorCard
          message="Question paper generation failed. Please retry to generate a new paper."
