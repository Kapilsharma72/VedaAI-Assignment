...
function PageFooter() {
    return (<Text style={styles.footer} render={({ pageNumber, totalPages }) => `Page ${pageNumber} of ${totalPages}`} fixed/>);
}
function QuestionItem({ question }: {
    question: IQuestion;
}) {
    return (<View style={styles.questionRow} wrap={false}>
      ...
      <View style={styles.questionBody}>
        ...
        {question.options && question.options.length > 0 && (<View style={{ marginTop: 5 }}>
            {question.options.map((opt) => (<View key={opt.label} style={styles.optionRow}>
...
function SectionBlock({ section }: {
    ...
    return (<View style={styles.sectionBlock}>
      ...
      {section.questions.map((q) => (<QuestionItem key={q.number} question={q}/>))}
    </View>);
}
function AnswerKeyRow({ entry }: {
    entry: IAnswerKeyEntry;
}) {
    return (<View style={styles.answerKeyRow} wrap={false}>
    ...
    return (<Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.studentInfoBlock}>
          {(['Name', 'Roll Number', 'Class / Section'] as const).map((label) => (<View key={label} style={styles.studentInfoRow}>
              <Text style={styles.studentInfoLabel}>{label}:</Text>
              <View style={styles.studentInfoLine}/>
            </View>))}
        </View>

        {paper.instructions ? (<View style={styles.instructionsBlock}>
            <Text style={styles.instructionsTitle}>General Instructions:</Text>
            <Text style={styles.instructionsText}>{paper.instructions}</Text>
          </View>) : null}

        {paper.sections.map((section: ISection) => (<SectionBlock key={section.title} section={section}/>))}
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        {paper.answerKey.map((entry: IAnswerKeyEntry) => (<AnswerKeyRow key={entry.questionNumber} entry={entry}/>))}
      </Page>
    </Document>);
}
