...
function AIToolkitIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
...
/**
 * Sidebar — persistent left-hand navigation for md+ viewports.
 *
 * - Background: #1a1f2e (sidebar design token)
 * - Hidden on viewports < md via `hidden md:flex`
 * - Assignments nav item shows a badge with the current assignment count
 * - Logout clears the auth store and redirects to /login
 */
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 flex-shrink-0 flex-col bg-[#1a1f2e] text-white">
      {/* Brand / Logo */}
          ...
          return (
            ...
            >
              ...
              {showBadge && assignmentCount > 0 && (
                <span className="ml-auto flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-[#6366f1] px-1.5 text-xs font-semibold text-white">
