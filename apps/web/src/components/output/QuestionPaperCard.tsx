...
function DifficultyBadge({ difficulty }: {
    ...
    return (<span className={[
            ...
            config.className,
        ].join(' ')} aria-label={`Difficulty: ${config.label}`}>
      {config.label}
    </span>);
}
function ChevronIcon({ open }: {
    ...
    return (<svg className={[
        ...
        ].join(' ')} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden="true">
      <path strokeLinecap="round" strokeLinejoin="round" d="m19 9-7 7-7-7"/>
    </svg>);
}
export default function QuestionPaperCard({ paper }: QuestionPaperCardProps) {
    const [answerKeyOpen, setAnswerKeyOpen] = useState(false);
    return (<article className="w-full rounded-[0.75rem] border border-gray-200 bg-white shadow-sm print:shadow-none print:border-none" aria-label="Generated question paper">
      <div className="px-8 py-8 sm:px-12 sm:py-10">
        ...
        {paper.instructions && (<div className="mt-4 rounded-md bg-gray-50 px-4 py-3 text-xs text-gray-600 leading-relaxed">
            <p className="font-semibold text-gray-700 mb-1">General Instructions:</p>
            <p className="whitespace-pre-line">{paper.instructions}</p>
          </div>)}

        {paper.sections.map((section) => (<section key={section.title} className="mt-6" aria-label={`Section: ${section.title}`}>
            ...
            <ol className="mt-3 space-y-3" aria-label={`Questions in ${section.title}`}>
              {section.questions.map((question) => (<li key={question.number} className="flex items-start gap-3 text-sm text-gray-800">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {question.options && question.options.length > 0 && (<ul className="mt-2 space-y-1 pl-1" aria-label="Answer options">
                        {question.options.map((opt) => (<li key={opt.label} className="flex items-start gap-2 text-sm text-gray-700">
        ...
        <div className="mt-8 border-t border-gray-300 pt-4">
          <button type="button" onClick={() => setAnswerKeyOpen((prev) => !prev)} aria-expanded={answerKeyOpen} aria-controls="answer-key-content" className="flex w-full items-center justify-between rounded-md px-1 py-1 text-sm font-semibold text-gray-700 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6366f1] focus-visible:ring-offset-2 transition-colors">
            <span className="uppercase tracking-widest text-xs">Answer Key</span>
            <ChevronIcon open={answerKeyOpen}/>
          </button>

          {answerKeyOpen && (<div id="answer-key-content" role="region" aria-label="Answer Key" className="mt-3 rounded-md bg-gray-50 px-4 py-4">
              {paper.answerKey.length === 0 ? (<p className="text-xs text-gray-500 italic">No answer key available.</p>) : (<ol className="space-y-1.5">
                  {paper.answerKey.map((entry) => (<li key={`${entry.sectionTitle}-${entry.questionNumber}`} className="flex items-start gap-2 text-xs text-gray-700">
