...
/**
 * AssignmentCard — displays a single assignment in the assignments grid.
 *
 * - Clicking the card navigates to /assignments/:id
 * - The delete button triggers optimistic removal from the store, then calls
 *   DELETE /api/assignments/:id; on error the card is restored and a toast is shown
 * - Status badge is color-coded: pending=gray, processing=blue, completed=green, failed=red
 */
export default function AssignmentCard({ assignment }: AssignmentCardProps) {
  ...
  const handleDelete = async (e: React.MouseEvent<HTMLButtonElement>) => {
    // Prevent the Link wrapper from navigating when the delete button is clicked
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <Link
      ...
      className="group block rounded-[0.75rem] border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6366f1] focus-visible:ring-offset-2"
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="flex-shrink-0 flex h-8 w-8 items-center justify-center rounded-md text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
        >
